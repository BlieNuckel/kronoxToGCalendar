...
The first time the script is run it must be run either from an IDE or through some other terminal. This is important, as whenthe script generates a config file based on several terminal inputs on the first run. Besides this the first run will also generate a Google authentication token, where the scripts will request access to the relevant Google account's calendars.
