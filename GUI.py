...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        tk.Label(root, text="Icalendar URL (from kronox)").grid(
            row=3, column=0
        )
        self.ical_url = tk.Text(root, height=1, width=52)
        self.ical_url.grid(row=4, column=0)

        tk.Label(root, text="Language of the programme you attend").grid(
            row=5, column=0
        )
        ...
        for (text, value) in lang.items():
            tk.Radiobutton(
                frame, text=text, variable=lang_var, value=value
        ...
        self.discord_choice = tk.Checkbutton(
            root, variable="discord_choice_var", offvalue=False, onvalue=True
        )
        self.discord_choice.grid(row=8, column=0)
        discord_choice_var.trace_add(
            "write",
        ...
        self.add_to_startup_label = tk.Label(
            root, textvariable=self.startup_text
        )
        self.add_to_startup = tk.Checkbutton(
            root, variable="add_to_startup_var", offvalue=False, onvalue=True
        )
        self.add_to_startup_label.grid(row=10)
        self.add_to_startup.grid(row=11)
        add_to_startup_var.trace_add(
            "write",
        ...
        self.confirm_button = tk.Button(
            confirm_button_frame, text="Confirm", command=self.confirm_pressed
        )
